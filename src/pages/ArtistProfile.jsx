...
export default function ArtistProfile() {
  ...
  const [nome] = useState(() => localStorage.getItem("artfolio_nome") || "Marina Silva");
  const [biografia] = useState(() => localStorage.getItem("artfolio_biografia") || "Explorando a intersecção entre o artesanato têxtil e a modelagem 3D. Baseada em São Paulo, transforma sentimentos em formas tangíveis desde 2018.");
  const [cidade] = useState(() => localStorage.getItem("artfolio_cidade") || "São Paulo");
  const [estado] = useState(() => localStorage.getItem("artfolio_estado") || "SP");
  const [email] = useState(() => localStorage.getItem("artfolio_email_publico") || "contato@marinasilva.com");
  const [plano] = useState(() => localStorage.getItem("artfolio_plano") || "FREE");
  
  const [categoria] = useState(() => {
    const raw = localStorage.getItem("artfolio_categoria_principal") || "pintura-digital";
    const map = {
      "pintura-digital": "Pintura Digital",
      "modelagem-3d": "Modelagem 3D",
      "textil": "Têxtil",
      "artesanato": "Artesanato",
      "desenho": "Desenho Manual"
    };
    return map[raw] || "Pintura Digital";
  });
            ...
            <aside className="lg:col-span-3 space-y-5">
              ...
              <div className="bg-artDark rounded-[1.7rem] p-5 text-white relative overflow-hidden">
                ...
                <h3 className="font-editorial text-2xl italic leading-tight">
                  {plano === "PRO" ? "Perfil com prioridade no feed." : "Perfil básico (Gratuito)."}
                </h3>

                <p className="text-xs text-gray-400 mt-3 leading-relaxed">
                  {plano === "PRO"
                    ? "Suas obras aparecem com mais destaque para visitantes e colecionadores."
                    : "Faça upgrade para destacar seu portfólio no feed."}
                </p>
